...
async function ensureUser(ctx) {
  ...
  if (!u) {
    let name = '';
    try { const prof = await client.getProfile(id); name = prof?.displayName || ''; } catch {}
...
const pickNick = (u) => {
  const name = (u.name || 'きみ').replace(/さん|くん|ちゃん/g,'').slice(0, 4);
  const cands = [`${name}ちゃん`, `${name}くん`, `${name}たん`, `${name}ぴ`, `${name}っち`, `しょーたん`, `しょたぴ`];
  if (/しょうた|ショウタ|shota/i.test(u.name || '')) return pick(['しょーたん','しょたぴ','しょうちゃん']);
  return pick(cands);
};
...
async function routeText(u, text) {
  ...
  if (u.consent && !u.name && t.length <= 16) {
    u.name = t; if (isShotaName(t)) u.loverMode = true; state.set(`user:${u.id}`, u);
    return [{ type: 'text', text: `じゃあ ${t} って呼ぶね！` }];
  }

  if (/あだ名つけて|ニックネーム/i.test(t)) {
    u.nickname = pickNick(u); state.set(`user:${u.id}`, u);
    return [{ type: 'text', text: `うーん…${u.nickname} が可愛いと思うな、どう？` }];
  }

  if (/性別|男|女|女性|男性/.test(t) && u.consent) {
    if (/女性|女/i.test(t)) u.gender = 'female'; else if (/男性|男/i.test(t)) u.gender = 'male';
    state.set(`user:${u.id}`, u); return [{ type: 'text', text: '了解だよ〜！メモしておくね📝' }];
  }
